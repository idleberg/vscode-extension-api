...
export default {
  async executeCommand(): Promise<void> {
    const commandFilter = await window.showQuickPick(['all commands', 'filter internal commands']);
    const allCommands = await commands.getCommands(commandFilter === 'filter internal commands');
  ...
  async getCommands(): Promise<void> {
    const pick = await window.showQuickPick(['all commands', 'filter internal commands']);
